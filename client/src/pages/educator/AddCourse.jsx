...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const {data} = await fetch(backendUrl + '/api/educator/add-course',formData,{
        headers: {
          Authorization:{
  ...
  return (
    <div className='h-screen flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0 overflow-scroll'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 w-full max-w-md text-gray-500'>
        <div className='flex flex-col gap-1'>
          ...
          <input 
            ...
            className='outline-none border border-gray-500 md:py-2.5 py-2 px-3 rounded' 
            required
          />
        </div>
        <div className='flex flex-col gap-1'>
          <label>Course Description</label>
          <div ref={editorRef} className="h-64 mb-12"></div>
        </div>
        ...
        <div className='flex flex-col gap-1'>
          <p>Discount %</p>
          <input type="number" onChange={e => setDiscount(e.target.value)} value={discount} placeholder='0' min={0} max={100} className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500 w-28' />
        </div>
